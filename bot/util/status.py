...
class ApiBasis:
    ...
    def __init__(
        self, url: dict, auth: str, headers: dict = None, payload: dict = None
    ):
...
class DiscordBoats(ApiBasis):
    __name__ = "discord.boats"
    url = "https://discord.boats/api/v2/bot/{id}"

    def __call__(self, guildCount):
        return optional(**{"server_count": guildCount.Count,})
...
class guildCount:
    ...
    def to_dict(self):
        return {key.lower(): getattr(self, key, None) for key in self.__slots__}
...
class status_handler(object):
    ...
    def post(service, guilds_payload):
        try:
            r = post(
                service.url, json=service(guilds_payload), headers=service.headers,
            )
        except RequestException as e:
            log.debug("Failed to post server count " f"to {service.__name__}: {e}")
        else:
    ...
    def update_presence(self, guilds_payload):
        if self.presence:
            ...
            self.bot.client.update_presence(
                Status.online, Activity(type=ActivityTypes.listening, name=presence,)
            )
